function F = first_lowrank(A,N_j1,N_j2,N_j3,N_j4,B,alpha,N,g)
[~,u] = size(B);
t = -2*alpha;
...
for d=1:g
    if d==1
        DD_1 = M1_integral(alpha,N,u);%求M1
        FF1{1,1} = X_i{1,1}*B;
        for m=2:N
       ...
       % H = N_j1*F{1,d-1}
        ...
        H{1,3} = N_j3*F{1,d-1};
        H{1,4} = N_j4*F{1,d-1};
        FF2{1,1} = X_i{1,1}*cell2mat(H);%930*32
        v = size(F{1,1},2);
        DD_2 = M2_integral(alpha,N,u,v);
        for z=2:N
            LLL_i=cell(1,z-1);
            FFF_i=cell(z-1,1);
            for zz=1:z-1
                LLL_i{1,zz}=L_i{1,z+1-zz};
                FFF_i{zz,1}=FF2{1,zz};%930*32
            end
            LLL_ii = cell2mat(LLL_i);
            FFF_ii = cell2mat(FFF_i);
            FDD=LLL_ii*FFF_ii;%930*32
            FF2{1,z}=-L_i{1,1}^(-1)*FDD;%930*32
        end
        FF2;
        F{1,d} = cell2mat(FF2)*DD_2;
        %Q:这里是cell2mat(FF2)*DD_1还是cell2mat(FF2{1,2})*DD_1？？？
    end
end
end